...
import {
  ...
  ArrowLeftRight, ShoppingBag, Zap, Car, Film, Heart, Wallet, Download, Upload, LogOut,
} from "lucide-react";
...
export default function FinanceTracker({ initialData, onChange, onLogout, userEmail } = {}) {
  ...
  const [rates, setRates] = useState(initialData?.rates ?? { [todayStr()]: 775 });
  const [primary, setPrimary] = useState(initialData?.primary ?? "THB");
  const [splitPct, setSplitPct] = useState(initialData?.splitPct ?? { Needs: 50, Wants: 30, Savings: 20 });
  const [newRateDate, setNewRateDate] = useState(todayStr());
  const [newRateValue, setNewRateValue] = useState("");
  const fileInputRef = useRef(null);

  // If a parent (e.g. a login wrapper) passed onChange, push state up whenever
  // the persisted data actually changes, debounced so we don't write on every keystroke.
  React.useEffect(() => {
    if (!onChange) return;
    const t = setTimeout(() => {
      onChange({ transactions, rates, splitPct, primary });
    }, 600);
    return () => clearTimeout(t);
  }, [transactions, rates, splitPct, primary, onChange]);
        ...
        <div className="h-full overflow-y-auto pb-24" style={{ scrollbarWidth: "none" }}>
          ...
          {view === "settings" && (
            <div className="px-4 pt-6">
              ...
              {onLogout && (
                <div className="rounded-2xl p-4 mb-4" style={{ background: C.card }}>
                  <p className="text-sm font-semibold mb-1" style={{ color: C.ink }}>Account</p>
                  {userEmail && <p className="mb-3" style={{ color: C.faint, fontSize: 11 }}>Signed in as {userEmail}</p>}
                  <button onClick={onLogout}
                    className="flex items-center justify-center gap-2 w-full px-3 py-2.5 rounded-xl text-sm font-semibold" style={{ background: C.bg, color: C.ink }}>
